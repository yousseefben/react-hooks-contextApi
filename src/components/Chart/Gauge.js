import './chart.css';

import { Axis, Chart, Coord, Geom, Guide, View } from 'bizcharts';
import React, { Component } from 'react';

const { Text } = Guide;

const data1 = [];
for (let i = 0; i < 50; i++) {
  const item = {};
  item.type = `${i}`;
  item.value = 10;
  data1.push(item);
}

const data2 = [];
for (let i = 0; i < 50; i++) {
  const item = {};
  item.type = `${i}`;
  item.value = 10;
  if (i === 35) {
    item.value = 14;
  }
  if (i > 35) {
    item.value = 0;
  }
  data2.push(item);
}

const cols = {
  type: {
    range: [0, 1],
  },
  value: {
    sync: true,
  },
};

const colsView2 = {
  type: {
    tickCount: 3,
  },
};

class GaugeTick extends Component {
  render() {
    return (
      <Chart data={[1]} scale={cols} className="chart1" padding={[-150, -90]}>
        <View data={data1}>
          <Coord type="polar" startAngle={-9 / 8 * Math.PI} endAngle={1 / 8 * Math.PI} radius={0.8} innerRadius={0.75} />
          <Geom type="interval" position="type*value" color="rgba(0, 0, 0, 0.09)" size={6} />
        </View>
        <View data={data1} scale={colsView2}>
          <Coord type="polar" startAngle={-9 / 8 * Math.PI} endAngle={1 / 8 * Math.PI} radius={0.55} innerRadius={0.95} />
          <Geom type="interval" position="type*value" color="rgba(0, 0, 0, 0.09)" size={2} />
          <Axis
            name="type"
            visible= {false}
          />
          <Axis name="value" visible={false} />
        </View>
        <View data={data2} >
          <Coord type="polar" startAngle={-9 / 8 * Math.PI} endAngle={1 / 8 * Math.PI} radius={0.8} innerRadius={0.75} />
          <Geom type="interval" position="type*value" color={['value', '#3023AE-#53A0FD']} opacity={1} size={6} />
          <Guide>
            <Text
              position={['50%', '58%']}
              content="70"
              style={{
                fill: '#262626',
                fontSize: 120,
                fontWeight: 300,
                textAlign: 'center',
                textBaseline: 'middle',
              }}
            />
            <Text
              position={['50%', '71%']}
              content="Score"
              style={{
                fill: '#262626',
                fontSize: 70,
                fontWeight: 300,
                textAlign: 'center',
                textBaseline: 'middle',
              }}
            />
          </Guide>
        </View>
      </Chart>
    );
  }
}
export default GaugeTick;